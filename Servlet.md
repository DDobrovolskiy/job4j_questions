...
Эти два метода могут давать разные результаты только в том случае, если контейнер приложения использует параметры URL для передачи сеанса ID. Поскольку в наши дни почти все используют для этого файлы cookie, маловероятно, что вы заметите разницу в своем обычном тестировании.

Чтобы принудительно включить сеанс IDs в URLs, либо отключите хранение файлов cookie сеанса в вашем браузере (и надейтесь, что ваш сервер приложений обнаружит этот факт), либо явно включите сеанс IDs в URLs на вашем сервере приложений.
